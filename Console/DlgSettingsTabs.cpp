...
LRESULT DlgSettingsTabs::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	m_listCtrl.InsertColumn(0, L"Tab name", LVCFMT_LEFT, 50, 0);
	...
	TCITEM tcItemMain;
	tcItemMain.mask = TCIF_TEXT;
	tcItemMain.pszText = _T("Main");

	TCITEM tcItemBackground;
	tcItemBackground.mask = TCIF_TEXT;
	tcItemBackground.pszText = _T("Background");
	
	m_tabCtrl.InsertItem(0, &tcItemMain);
	m_tabCtrl.InsertItem(1, &tcItemBackground);
